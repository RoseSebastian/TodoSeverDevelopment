...
const Todo = () => {
  ...
  const handleToggleComplete = (id) => {
    ...
    axios.put(`http://localhost:3000/todos/${id}`, { ...todo, completed: !todo.completed })
      .then(response => {
      ...
      <ul>
        {todos?.map(todo => (
          <li key={todo.id} className={todo.completed ? 'completed' : 'pending'}>
            ...
            <Button variant="primary" onClick={() => handleToggleComplete(todo.id)}>Mark as Completed</Button>
            </div>
            
            <p>{todo.description}</p>
            <Button variant="warning" onClick={() => handleDeleteTodo(todo.id)}>Delete</Button>
          </li>
        ))}
      </ul>
    </div>
